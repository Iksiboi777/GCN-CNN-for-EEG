...
# Checklist #6 & #13: Standard names matched to your new Preprocessing pipeline
STANDARD_CH_NAMES = [
    'Fp1', 'Fpz', 'Fp2', 'AF3', 'AF4', 'F7', 'F5', 'F3', 'F1', 'Fz', 'F2', 'F4', 'F6', 'F8',
    'FT7', 'FC5', 'FC3', 'FC1', 'FCz', 'FC2', 'FC4', 'FC6', 'FT8', 'T7', 'C5', 'C3', 'C1',
    'Cz', 'C2', 'C4', 'C6', 'T8', 'TP7', 'CP5', 'CP3', 'CP1', 'CPz', 'CP2', 'CP4', 'CP6',
...
def get_knn_adjacency_matrix(locs_file_path=None, k=5):
    ...
    """
    # 1. Load positions using the MNE standard (more robust than custom .locs)
    try:
        montage = mne.channels.make_standard_montage('standard_1020')
        # Map SEED specific names to montage standard
    ...
    # 2. Extract positions in the exact order of your Features
    positions = []
    for name in STANDARD_CH_NAMES:
        # Check standard name or mapped name
        lookup = name if name not in ['CB1', 'CB2'] else ('PO9' if name == 'CB1' else 'PO10')
        if lookup in pos_dict:
            positions.append(pos_dict[lookup])
        else:
            # If still not found, use a zero-vector (will be fixed by k-NN)
            positions.append([0, 0, 0])
            print(f"Warning: Channel {name} location synthesized.")
            
    positions = np.array(positions) # (62, 3)
    num_nodes = len(positions)
    
    # 3. Calculate Pairwise Distances (Euclidean)
    dists = np.zeros((num_nodes, num_nodes))
    for i in range(num_nodes):
        for j in range(num_nodes):
            dists[i, j] = np.linalg.norm(positions[i] - positions[j])
    ...
    edges = []
    for i in range(num_nodes):
        # Sort indices by distance. Index 0 is self, so take 1 to k+1
        sorted_indices = np.argsort(dists[i])
        nearest_neighbors = sorted_indices[1 : k + 1]
    ...
    # 5. Convert to PyG Format
    edge_index = torch.tensor(edges, dtype=torch.long).t().contiguous()
    
    # CHECKLIST #14: Add Self-Loops (Node sees itself)
    edge_index, _ = add_self_loops(edge_index, num_nodes=num_nodes)
