...
class SEBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        global_feat = x.mean(dim=1) 
        # Excitation (Weights calculation)
        importance = self.fc(global_feat).unsqueeze(1) 
        return x * importance
...
class Adaptive_DGCNN(nn.Module):
    def __init__(self, static_adj, num_nodes=62, in_features=10, hidden_dim=128, 
        ...
        self.use_se = use_se
        self.use_doubling = use_doubling
        # 1. Fizikalni prior (A_static) - registriramo kao buffer da se ne trenira ali prati uređaj
        self.register_buffer('static_adj', static_adj)
        
        # Naučeni balans između statičkog i dinamičkog grafa
        ...
        # 3. Dynamic Topology Learner (Query-Key mechanism)
    ...
    def forward(self, x, subject_ids=None):
        # --- FAZA 1: Kalibracija senzora ---
        x = self.adaptive_input(x)
        if self.use_se:
            x = self.se_block(x)
        
        # --- FAZA 2: Učenje topologije (Dinamički + Statički prior) ---
        Q = self.weight_q(x)
        K = self.weight_k(x)
        
        # Dinamički izračun bridova
        A_dyn = torch.bmm(Q, K.transpose(1, 2))
        A_dyn = F.relu(A_dyn) # Sparsity: uklanjanje negativnih korelacija
        
        # Integracija sa statičkim fizikalnim priorom
        # A = (1-alpha)*A_static + alpha*A_dynamic
        ...
        for i in range(self.num_layers):
            x = self.convs[i](x, A)
            x = self.norms[i](x) # Stabilizacija po čvorovima
            x = F.relu(x)
            x = self.dropout(x)

        # --- FAZA 4: Pooling (Attention sum) ---
        ...
        # --- FAZA 5: Subjektni pomak (Subject Bias) ---
        if subject_ids is not None:
            logits = logits + self.subject_bias(subject_ids)
